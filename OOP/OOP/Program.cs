...
namespace OOP
{
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // - 구조체라도 "어디에 담겼느냐"에 따라 스택 or 힙이 결정.
            // 1. 지역 변수 (메서드 안에서 생성) -> 스택 
            // 2. 클래스의 멤버 변수 (클래스 안에서 생성) -> 힙
            // 3. 배열의 원소 (배열 안에서 생성) -> 힙(배열 자체가 힙, 내부 요소도 힙)
            // 4. 박싱(Boxing)될 때 -> 힙
            // 하지만 클래스는 항상 힙에 생성된다.
            ...
            // - Call by Reference은 값을 복사하지 않고, 변수의 "주소 자체를 전달"하는 것이지만
            // - 그 주소가 꼭 힙에 있다는 보장은 없다. → 스택일 수도 있다!

            // 따라서 Call by Reference는 메모리의 위치(스택/힙)와는 별개로, 값의 복사가 아닌 주소 전달을 통해 원본에 접근하는 방식임을 기억하면 된다.

            // 그리고 “함수 안에서 객체를 new 하는 것”과 “클래스 멤버 변수로 선언해두고, 나중에 메모리 할당하는 것”의 차이
            // 1. 함수 안에서 new 하는 것
            // - 함수가 끝나면 메모리 해제됨
            // - 하지만 new로 만든 소켓 인스턴스는 GC가 회수할 때까지는 살아있음
            // - 그러나 다른 함수에서 이 socket에 접근 못 함 ❌
            /* 
            void Connect()
            {
            ...
            */
            // 이런 패턴은 일회성 작업이나, 고립된 연결 시도에 적합

            // 2. 클래스 멤버 변수로 선언해두고 나중에 메모리 할당(new)하는 것
            // - _socket은 Session 인스턴스가 살아있는 동안 계속 존재
            // - 여러 함수에서 공유하며 상태를 유지하고 재사용 가능 
            // - 연결 유지, 리시브 처리, 디스커넥트 등에서도 모두 접근 가능
            // - 📌 이건 * *“이 소켓이 내 전용이다”**라는 구조에 적합함
            // - 그리고 장기 연결 유지, 상태 관리, 재사용 필요할 때 적합.
