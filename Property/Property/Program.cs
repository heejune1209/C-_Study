...
{
    // Property(속성)
    // 외부에서 값을 바꾸면 그 값을 누가 언제 바꿨는지 알기가 쉽지 않다.
    // 따라서 외부에서 write되지 않기를 원하는 멤버 변수는 protected, private 으로 접근을 제한하고 set, get 같은 접근 함수를 public으로 만들어서 이들에 대해 간접 접근한다.
    // (이런 경우 디버깅시 호출 스택을 통해 어디서 누가 호출했는지 쉽게 알 수 있다.)

    // 프로퍼티는 은닉성과 관련이 있으며 Get, Set 접근 함수와 같은 기능을 좀 더 편하게 사용하기 위한 기능이다.
    internal class Program
    {
        class Knight
        {
            ...
            // ✔ 프로퍼티의 정의다. set에서 value는 Write 하려는 새로운 값을 뜻한다.
            ...
            //{
            //    get { return hp; }
            // get, set 中 하나만 정의해도 무방하다.
            // get만 정의했다면 set은 정의하지 않았기 때문에 knight.Hp = 100;은 불가능하다.(컴파일 에러) knight.Hp를 리턴받는 것만 가능하다.
            //}
            ...
            // 프로퍼티를 위와 같이 구현하면 멤버 변수를 직접 만들어 줄 필요가 없다.
            // 컴파일러가 private한 멤버 변수까지 자동으로 생성시켜주기 때문이다. 간단하게 get; set; 만 해주면 자동 완성 프로퍼티다.
            // 이렇게 정말 기본적인 Get, Set 작업만을 할 때 사용 가능하다. 뭔가 더 사용자 지정 코드를 작성할 경우에는 자동 완성 프로퍼티를 사용할 수 없다.
            ...
            //public int Hp { get; set; } = 100; // 더 나아가 이렇게 초기화도 가능하다. 아래 코드 3 줄과 같다.           
            //private int _hp = 100;
            //public int GetHp() { return _hp; }
            //public void SetHp(int value) { _hp = value; }
        }
