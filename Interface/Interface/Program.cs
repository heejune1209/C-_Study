...
namespace Interface
{
    ...
    // 가상 함수와는 달리 구현부{}
    // 를 작성하면 안된다. 함수의 인터페이스만 정의한다. 그래서 추상적으로 존재만 하는 함수라 추상 함수다!
    ...
    // 1️. Shout()를 새롭게 오버라이딩 한다.
    // - 문제 없다. 자신만의 내용으로서 새롭게 정의하는 것이기 때문에.
    // 2️. 부모인 Orc,Skeleton 로부터 물려받은 Shout()를 오버라이딩하지 않고 물려받은 그대로 사용한다.
    // 👉 이 경우 문제 발생 !!
    ...
    // - 죽음의 다이아몬드 문제 👉 그대로 상속 받아 쓰려고 할 때,
    // 두 부모가 동일한 함수이나 오버라이딩으로 구현부가 다른 함수를 가지고 있기 때문에 오버라이딩 하지 않고 그대로 물려 받아 사용하려고 할 때 어떤 것을 사용해야할지 애매해지는 문제

    // 인터페이스
    // 추상함수’만’ 가지고 있는 클래스.(상태(변수)를 가지지 않는다. 기능만 정의)
    ...
    // 따라서 인터페이스를 다중 상속 받을 땐 죽음의 다이아몬드 문제가 발생하지 않는다. (선택지를 1️로만 강제함)

    // 추가로 C#에서도 인터페이스가 인터페이스를 상속할 수 있어요.
    public interface IMovable
    {
        void Move();
    }

    public interface IFlyable2 : IMovable
    {
        void Fly();
    }
    // IFlyable2은 IMovable을 상속했으니, IFlyable2을 구현하는 클래스는 Move()와 Fly() 둘 다 구현해야 해요.
    ...
    //class SkeletonOrc : Orc, Skeleton // ❌❌ 컴파일 에러 ! 다중 상속 불가능
    //{
    ...
    internal class Program
    {
        ...
        {
            flyable.Fly(); // 아무 문제 없다. 자식 -> 부모 (O)
        }

        static void Main(string[] args)
        {
            // Monster monster = new Monster(); // ❌❌ 컴파일 에러! 추상 클래스 타입의 객체는 만들 수 없다.
            Monster monster = new Orc(); // ⭕ 다형성, 자식 -> 부모 (O)
